...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"./shaders/light_vs.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"./shaders/light_ps.hlsl");
	if (error != 0)
	{
		return false;
	}
...
void LightShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, L"Error compiling shader.  Check light-shader-error.txt for message.", shaderFilename, MB_OK);
