...
bool NormalMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"./shaders/normalmap_vs.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"./shaders/normalmap_ps.hlsl");
	if (error != 0)
	{
		return false;
	}
...
void NormalMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	fout.open("normalmap-shader-error.txt");
	...
	MessageBox(hwnd, L"Error compiling shader.  Check normalmap-shader-error.txt for message.", shaderFilename, MB_OK);
