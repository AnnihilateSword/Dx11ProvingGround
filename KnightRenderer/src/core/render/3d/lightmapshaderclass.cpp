...
bool LightMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"./shaders/lightmap_vs.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"./shaders/lightmap_ps.hlsl");
	if (error != 0)
	{
		return false;
	}
...
void LightMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	fout.open("lightmap-shader-error.txt");
	...
	MessageBox(hwnd, L"Error compiling shader.  Check lightmap-shader-error.txt for message.", shaderFilename, MB_OK);
